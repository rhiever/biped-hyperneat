...
NNode::NNode(nodetype ntype,int nodeid) {
	...
	node_id=nodeid;
	ftype=(functype)(rand()%ACTIVATION_FUNCTION_END); // Randomly select activation function
	nodetrait=0;
...
NNode::NNode (const char *argline, std::vector<Trait*> &traits) {
	...
	ss >> node_id >> traitnum >> nodety >> nodepl >> time_const >> bias >> afunc >> x_sub_pos >> y_sub_pos >> z_sub_pos;
